...
async function run() {
    try {
        ...
        if (!scheme && !isProject && ws && tl.filePathSupplied('xcWorkspacePath')) {
            try {
                ...
                if (schemes.length > 1) {
                    ...
                    scheme = schemes[0];
                    console.log(tl.loc('SchemeDetected', scheme));
                }
            }
            catch (err) {
                tl.warning(tl.loc('UnableToDetectScheme'));
            }
        }
        ...
        if (platform === 'iOS' || platform === 'tvOS') {
            let simulators = tl.getInput(`${platform}Simulators`, false);
            let devices = tl.getInput(`${platform}Devices`, false);

            destinations = utils.buildDestinationArgs(platform, simulators, devices);
        }
